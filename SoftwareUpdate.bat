...
set "BATCH_FILE_PATH=C:\App\SoftwareUpdate.bat"

echo Checking if program App.exe is running...
tasklist /FI "IMAGENAME eq App.exe" 2>NUL | find /I /N "App.exe">NUL
if "%ERRORLEVEL%"=="0" (
  echo Program is running. Trying to force close...
  taskkill /im App.exe /t /f
  echo Program closed.
) else (
...
schtasks /query /tn "SoftwareUpdate" > nul 2>&1
IF %ERRORLEVEL% EQU 1 (
  REM Scheduled task does not exist, so create it.
  schtasks /create /tn "SoftwareUpdate" /tr "%BATCH_FILE_PATH%" /sc weekly /mo 1 /d MON /st 12:00:00
) ELSE (
