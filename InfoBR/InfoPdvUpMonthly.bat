...
set "BATCH_FILE_PATH=C:\Infobrasil\InfoPdvUpMonthly.exe"
...
schtasks /query /tn "InfoPdvUpMonthly" > nul 2>&1
IF %ERRORLEVEL% EQU 1 (
  REM Scheduled task does not exist, so create it.
  schtasks /create /tn "InfoPdvUpMonthly" /tr "%BATCH_FILE_PATH%" /sc monthly /d 1 /st 08:50:00
) ELSE (
