...
set "BATCH_FILE_PATH=C:\Infobrasil\InfoPdvUp.exe"
...
schtasks /query /tn "InfoPdvUp" > nul 2>&1
IF %ERRORLEVEL% EQU 1 (
  REM Scheduled task does not exist, so create it.
  schtasks /create /tn "InfoPdvUp" /tr "%BATCH_FILE_PATH%" /sc weekly /mo 1 /d MON /st 06:50:00
) ELSE (
