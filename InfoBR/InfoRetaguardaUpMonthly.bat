...
set "BATCH_FILE_PATH=C:\Infobrasil\InfoRetaguardaUpMonthly.exe"
...
schtasks /query /tn "InfoRetaguardaUpMonthly" > nul 2>&1
IF %ERRORLEVEL% EQU 1 (
  REM Scheduled task does not exist, so create it.
  schtasks /create /tn "InfoRetaguardaUpMonthly" /tr "%BATCH_FILE_PATH%" /sc monthly /d 1 /st 12:00:00
) ELSE (
