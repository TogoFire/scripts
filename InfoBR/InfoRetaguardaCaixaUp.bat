...
set "BATCH_FILE_PATH=C:\Infobrasil\InfoRetaguardaCaixaUp.exe"
...
schtasks /query /tn "InfoRetaguardaCaixaUp" > nul 2>&1
IF %ERRORLEVEL% EQU 1 (
  REM Scheduled task does not exist, so create it.
  schtasks /create /tn "InfoRetaguardaCaixaUp" /tr "%BATCH_FILE_PATH%" /sc weekly /mo 1 /d MON /st 07:00:00
) ELSE (
